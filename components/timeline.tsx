        ...
        <div className="hidden lg:flex justify-between items-start relative px-4">
            ...
            {timelineEvents.map((event, idx) => (
                <div key={idx} className="flex flex-col items-center text-center space-y-4 w-60 relative">
                    <div className={`h-12 w-12 rounded-full flex items-center justify-center border-4 ${event.status === "done" ? "bg-primary border-primary/20 text-white" : "bg-background border-muted text-muted-foreground"}`}>
        ...
        <div className="lg:hidden space-y-8 pl-4 border-l-2 border-primary/20 ml-4 relative">
            {timelineEvents.map((event, idx) => (
                <div key={idx} className="relative pl-6">
                    <div className={`absolute -left-[33px] top-0 h-10 w-10 rounded-full flex items-center justify-center border-4 ${event.status === "done" ? "bg-primary border-primary/20 text-white" : "bg-background border-muted text-muted-foreground"}`}>
